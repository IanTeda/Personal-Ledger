...
- `?`'s help overlay is specified for `bin-desktop` (this document's own "Modes"/grammar implies it should exist) but has no ticket anywhere — flagged on the [Desktop Shell & Navigation](https://github.com/IanTeda/Personal-Ledger/issues/144) map's own "Not yet specified" already, not duplicated here as a separate open question.
