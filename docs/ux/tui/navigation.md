...
[`docs/getting-around.md`](../../getting-around.md) is the cross-client layer this document sits under — the keyboard grammar (leader keys, modes, what's configurable) the TUI and `bin-desktop` are meant to share. Read it first; this document is what's TUI-specific on top of it.
