...
Two more companion bundles sit alongside it and are **not yet built** — treat each `README.md` as the current source of truth for its view until a map lands it, and check the domain vocabulary against `CONTEXT.md` first (the Accounts bundle needed its type list reconciled with the glossary before it could be built):

- `docs/ux/desktop/Transactions/` — the `Noun::Transactions` cross-account ledger: a filter-chip header, a table with an ACCOUNT column and a RUNNING TOTAL over whatever is filtered (4a), and the filter-builder popover (4b). Distinct from the per-account ledger the Accounts page's `enter` will open (screen 1b of the Shell & Navigation bundle).
