...
`docs/ux/desktop/Accounts/` is a third, companion handoff bundle — the `Noun::Accounts` management surface (the full-width landing page, grouped by account type) and its three add/edit/delete dialogs — likewise not yet translated into a `gpui`-terms living spec; treat its `README.md` as the current source of truth for that view until it is.
...
`docs/ux/desktop/Accounts/` is a third companion bundle — the `Noun::Accounts` view's grouped management page and its Add, Edit and Delete dialogs. Like Settings it stayed the living spec in place; the [Desktop Accounts Surface](https://github.com/IanTeda/Personal-Ledger/issues/190) map has fully landed against `crates/bins/bin-desktop/src/view/accounts/`, closed out by that map's own acceptance pass — see its `README.md`'s "Acceptance pass" section, which also lists the deliberate departures from the mockup.
