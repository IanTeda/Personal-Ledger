...
`docs/ux/desktop/Categories/` is a fifth, companion handoff bundle — the `Noun::Categories` management table, a hierarchical tree up to three levels deep grouped Expense / Income with budget and spent-this-month columns (5a), and its Add, Edit and Delete dialogs (5b–5d), the same management-table pattern as Accounts. Like the others it stayed the living spec in place; the [Desktop Categories Surface](https://github.com/IanTeda/Personal-Ledger/issues/269) map has fully landed against `crates/bins/bin-desktop/src/view/categories/` on stubbed in-memory data, closed out by that map's own acceptance pass — see its `README.md`'s "Acceptance pass" section, which also lists the deliberate departures from the mockup.
