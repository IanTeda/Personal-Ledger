...
- **Public surface:** `init(&str) -> Locale`, `Locale`, the generated `msg::…` accessors, the rich segment types, `format_money(&Money, &Unit)`, `format_date(NaiveDate, DateStyle)`, `format_number`, `format_amount(&Money, AmountStyle) -> Amount` (signed amounts: U+2212 minus, a rounded zero never negative, optional `+`), `parse_date(&str, today)`, a Locale-aware `upper`, the `Label` trait, and a `with_locale(...)` test helper.
