...
Light vs dark is not a separate role set — it's a *direction*: base00–base07 "should range from dark to light" for a dark scheme and "from light to dark" for a light scheme ([styling guide](https://github.com/tinted-theming/home/blob/main/styling.md)). A scheme file carries a `variant: dark` / `variant: light` metadata field (per [`builder.md`](https://github.com/tinted-theming/home/blob/main/builder.md): "Currently only 'light' and 'dark' are used") rather than the scheme shipping two colour sets.
...
Contrast handling: no explicit contrast-ratio rule in either spec; the styling guide says only that the default scheme's colours "were chosen to be easily separable" and that "these are just guidelines", so correctness is by manual scheme authoring, not an enforced or computed check.
...
Omarchy's build step then template-substitutes these into per-app configs (Alacritty, Ghostty, Kitty, foot, btop, Hyprland, Neovim, Helix, VS Code, Waybar, …), so this flat set is the *lowest common denominator* Omarchy found sufficient to drive every one of those consumers. A newer "Quattro" schema (Omarchy 4.0+, seen in `omacom/omarchy` fork docs, not in the official manual or `basecamp/omarchy` `dev` — see [Unverified](#unverified)) widens this to named semantic slots — `mode`, `accent`, `background`/`dark_background`/`darker_background`/`lighter_background`, `foreground`/`dark_foreground`/`light_foreground`/`bright_foreground`, plus `red`/`yellow`/`orange`/`green`/`cyan`/`blue`/`magenta`/`brown` and their `bright_*` counterparts, with legacy `bg`/`fg` aliases kept for back-compat — this is worth flagging as the direction terminal-scheme-driven systems tend to evolve in once "16 ANSI colours" stops being enough for a whole desktop's theming. Light vs dark Colour Variant: the official manual documents a `mode = "light"` field at the top of `colors.toml`, or the older convention of dropping an empty `light.mode` marker file next to it.
...
Counting how many of the 14 systems in the table above name (or convention-map) each concept independently gives a rough recurrence signal, then grouping into families:
...
- Positive/success and negative/error/danger — named explicitly in Zed, VS Code (git-specific), `gpui-component`, `ratatui-themes`; convention-mapped (red/green) in base16/24, terminal schemes, Catppuccin. Strong candidate as two roles: `positive`/`negative` (repo vocabulary should pick one pair of terms and use it consistently — the #293 ticket text already says "positive/negative").
...
- Danger vs "negative" wording — worth flagging as a naming decision, not just a role: `gpui-component` says `danger`, most others say `error`/`negative`. The #293 ticket text uses "positive/negative", so that pairing is likely the house preference already.
...
- **Omarchy "Quattro" `colors.toml` keys.** The official [manual](https://omarchy.org/manual/making-your-own-theme/) documents `mode = "light"` and template placeholders such as `{{ red }}` alongside `{{ color0 }}`–`{{ color15 }}`, but not the `dark_background`/`lighter_background`/`bright_*` key list quoted in §2, and `basecamp/omarchy`'s `dev` `themes/nord/colors.toml` is still the flat pre-Quattro shape. Treat that key list as indicative of direction only.
