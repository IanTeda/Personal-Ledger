//! Personal Ledger desktop app -- feasibility-cycle demo (issue #23), kept compiling but no
//! longer wired into `main()` -- see ADR-0016 and the
//! [Desktop Shell & Navigation](https://github.com/IanTeda/Personal-Ledger/issues/144) map.
//! `Shell` (`crate::shell`) is the real navigation entry point now; this module is dead code,
//! not a live fallback path, the same treatment ADR-0013 gave the TUI's own `app.rs`/
//! `screen/`. Its `gpui-component` chart-widget usage is still the pattern real tickets on
//! that map reuse -- the `LineChart`/`PieChart` calls below are what the real Dashboard
//! view's chart band (issue #148) is built from.
//!
//! Demonstrated chart rendering in `GPUI` (ADR-0007) against dummy data, per
...
//! type and the table demo, so this feasibility cycle standardised on it rather than
...
//! `smol`-based (see `gpui`'s `Cargo.toml`), so `run` below runs under `#[tokio::main]` and
//! hands a `tokio::runtime::Handle` down to the load task -- `Handle::spawn` runs the actual
