...
//!
//! The negative sign is spelt once, here, as U+2212. Dates are lower case, as the views set them.
...
use lib_core::Money;
use lib_locale::format::{format_date, format_month_day, format_number, format_year_month};

/// The minus shown before a negative amount: U+2212, not a hyphen.
const MINUS: char = '\u{2212}';

/// An amount at exactly `decimal_places` (a Unit's own precision), grouped and marked by the
/// Locale, with [`MINUS`] for a negative. A value that rounds to zero is never negative.
pub fn money(value: &Money, decimal_places: i64) -> String {
    let places = u32::try_from(decimal_places.max(0)).unwrap_or(0);
    let negative = value.0.to_plain_string().starts_with('-');
    let magnitude = Money(value.0.abs());
    let text = format_number(&magnitude, places);
...
pub fn money_natural(value: &Money) -> String {
    money(value, if value.0.is_integer() { 0 } else { 2 })
}
...
mod tests {
    ...
    #[test]
    fn a_negative_carries_the_one_real_minus_and_a_zero_never_does() {
        with_locale(Locale::EnAu, || {
            assert_eq!(money(&amount("-18402.55"), 2), "\u{2212}18,402.55");
            assert_eq!(money(&amount("-0.004"), 2), "0.00");
            assert_eq!(money(&amount("-0.00"), 2), "0.00");
        });
    }
    ...
    fn chart_figures_go_through_the_same_formatter() {
        with_locale(Locale::EnAu, || {
            ...
            assert_eq!(money_f64(-3.5, 1), "\u{2212}3.5");
        });
    }
