//! # SyncUser Database Model
//!
//! Defines the `SyncUser` struct, one row of the Sync Server's auth user store
//! (`sync_users` table). Password hashing and JWT/refresh-token issuance are not this
...
pub struct SyncUser {
    ...
    /// session -- the sync user hasn't completed the OAuth2 flow, or was logged out.
    /// Rotates (replaced) on every successful token refresh.
    pub refresh_token_hash: Option<String>,

    /// UTC timestamp when the sync user was first created.
    pub created_on: chrono::DateTime<chrono::Utc>,

    /// UTC timestamp when the sync user was last modified.
    pub updated_on: chrono::DateTime<chrono::Utc>,
}

impl SyncUser {
    /// Generate a mock `SyncUser` instance with randomised test data.
    ///
    /// **Note**: This function is only available in test builds.
    #[cfg(test)]
    pub fn mock() -> Self {
        use crate::sync_users::SyncUserBuilder;
        use fake::Fake;
        ...
        SyncUserBuilder::new()
            ...
            .build()
            .expect("Mock SyncUser should always build successfully")
    }
}
...
mod tests {
    ...
    #[test]
    fn mock_generates_valid_sync_user() {
        let sync_user = SyncUser::mock();
        assert!(!sync_user.username.is_empty());
        assert!(!sync_user.password_hash.is_empty());
        assert!(sync_user.refresh_token_hash.is_none());
    }

    #[test]
    fn sync_user_struct_derives_work() {
        let a1 = SyncUser::mock();
        let a2 = a1.clone();
        assert_eq!(a1, a2);

        let debug_str = format!("{:?}", a1);
        assert!(debug_str.contains("SyncUser"));

        let json = serde_json::to_string(&a1).unwrap();
        let deserialized: SyncUser = serde_json::from_str(&json).unwrap();
